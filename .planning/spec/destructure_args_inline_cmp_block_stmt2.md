# Test: destructure_args_inline_cmp_block_stmt2
...
- **[CONV-01] QRL Calls**: `qrl(i_pFqTss400MA, ..., [props])` with capture array
- **[CONV-03] JSX Transforms**: `_jsxSorted("div", ...)` replaces JSX
- **[CONV-04] Signal Helpers**: `_fnSignal(_hf0, [props], _hf0_str)` wraps property comparison for reactive binding
- **[CONV-05] Capture Patterns**: `const props = _captures[0]` in entry point -- `props` is captured from parent scope
- **[CONV-06] Lazy Imports**: Dynamic import for event handler segment
- **[CONV-07] PURE Annotations**: `/*#__PURE__*/` on `_jsxSorted` and `qrl`
- **[CONV-08] Segment Extraction**: Event handler extracted to separate entry point
- **[CONV-11] Props Destructuring**: Unlike `destructure_args_inline_cmp_block_stmt`, here `props` is named (not destructured in parameter) so it keeps its original name. The `const { data } = props` destructuring in the body is removed -- `props.data` is accessed directly
- **[CONV-14] Hoisted Functions**: `_hf0` hoists property comparison expression
